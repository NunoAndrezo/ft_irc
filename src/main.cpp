...
/*   Updated: 2026/01/26 18:43:47 by nuno             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	//parse command-line arguments (port, password) // We need the server port and connection password because the server cannot start without knowing where to listen and what password to enforce
	//validate arguments // Ensures user input is correct so the server doesn’t crash or misbehave later. Check for valid port range and non-empty password So people like Hugo don't fuck with out project
	//initialize server state // Create data structures for clients, channels, and global configuration, because the server needs to track everything consistently
